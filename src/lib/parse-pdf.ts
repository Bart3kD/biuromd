...
export async function parsePdf(pdfPath: string): Promise<BankStatement> {
  ...
  for (const page of data.pages) {
    for (const item of page.content) {
      allItems.push({ x: item.x, y: item.y + yOffset, str: item.str });
    }
    yOffset += page.pageInfo.height + 10;
  }
